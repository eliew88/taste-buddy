/**
 * Meal Image Upload API with B2 Cloud Storage
 * 
 * Handles file uploads for meal images with validation and cloud storage.
 * Uses a separate "meals" folder in B2 storage to organize meal photos.
 * 
 * Features:
 * - File type validation (JPEG, PNG, WebP)
 * - File size validation (max 5MB)
 * - Unique filename generation
 * - B2 cloud storage with local fallback (meals folder)
 * - CDN URL generation
...
const UPLOAD_DIR = path.join(process.cwd(), 'public', 'images', 'meals');
...
function generateUniqueFilename(originalName: string): string {
  ...
  return `meal-${timestamp}-${randomString}${extension}`;
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!userId) {
      ...
      console.error('❌ Meal image upload failed - Authentication required', uploadLog);
    ...
    try {
      ...
      console.error('❌ Meal image upload failed - Invalid form data', uploadLog);
      return NextResponse.json(
    ...
    if (!file) {
      ...
      console.error('❌ Meal image upload failed - No image file provided', uploadLog);
      return NextResponse.json(
    ...
    if (!validation.valid) {
      ...
      console.error('❌ Meal image upload failed - File validation failed', uploadLog);
      return NextResponse.json(
    ...
    try {
      ...
      console.error('❌ Meal image upload failed - Buffer conversion failed', uploadLog);
      return NextResponse.json(
    ...
    // Step 6: Try B2 upload first (using "meals" folder)
    uploadLog.steps.push({ step: 'b2_upload', status: 'starting' });
    console.log('🔄 Attempting B2 meal image upload...');
    const b2Result = await uploadImageToB2(file, buffer, 'meals');
    
    if (b2Result.success && b2Result.url) {
      uploadLog.steps.push({ step: 'b2_upload', status: 'success', url: b2Result.url });
      console.log(`✅ Meal image uploaded to B2: ${b2Result.url} (${file.size} bytes)`);
    ...
    console.log('⚠️ B2 meal image upload failed, falling back to local storage...');
    ...
    const publicUrl = `/images/meals/${uniqueFilename}`;
    
    await writeFile(filePath, buffer);
    uploadLog.steps.push({ step: 'local_fallback', status: 'success', url: publicUrl });

    console.log(`✅ Meal image uploaded locally: ${publicUrl} (${file.size} bytes)`);
    ...
    console.error('❌ Meal image upload failed - Unexpected error:', error);
